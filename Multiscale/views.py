...
sys.path.insert(1, './Multiscale')
import complete_flow as flow
import write_html

# model_name = './Multiscale/Colab/best_model_DenseNet_DD.pkl'
# model = pickle.load(open(model_name, 'rb'))

#json_file = open('./Multiscale/Colab/model.json', 'r')
#loaded_model_json = json_file.read()
#json_file.close()
#model = model_from_json(loaded_model_json)
#model.load_weights("./Multiscale/Colab/model.h5")

model_name = './Multiscale/Colab/DenseNet_150_16_DD_FINALMODEL.h5'
...
def detected_and_segmented_image(save_state, bw_image, pred_result):
    ...
    for x in save_state:
        ...
        # cv2.rectangle(bw_copy, (coordinat[0], coordinat[1]),
        #               (coordinat[2], coordinat[3]),
        #               (0, 0, 0), 1)
...
def processing():
    ...
    if from_sketch_button:
        ...
        def runner():
            ...
            for global_count in range(len(list_image_files)):
                ...
                if max_id is None:
                    ...
                    listof_imagelist_template_scale_visualize.append(
                        imagelist_template_scale_visualize)
                    listof_imagelist_visualize_white_block.append(
                        imagelist_visualize_white_block)
                    ...
                    listof_normal_processing_result.append(
                        normal_processing_result)
                    listof_crop_ratio_processing_result.append(
                        crop_ratio_processing_result)
                    listof_imagelist_horizontal_line_by_eight_conn.append(
                        imagelist_horizontal_line_by_eight_conn)
                else:
                    ...
                    final_image_result, pred_result, char_recog, skip_index = flow.character_recognition(
            ...
            thefile = open('./Multiscale/templates/Saved Time.txt', 'a')
    ...
    for list_image in listof_imagelist_visualize_white_block:
        ...
        for font in list_image:
            for marker in font:
                for image in marker:
                    if image != []:
                        file = store_folder_wb+'/' + \
                            str(count1)+'_'+str(count2)+'.png'
                        if image != []:
                            cv2.imwrite(file, image)
                            path_image.append(file[35:])
                        count2 += 1
...
def processing_result():
    ...
    filename = "./Multiscale/static/dumpPath.pkl"
...
def sketch():
    ...
    if request.method == "POST":
        if request.is_json:
            ...
            if res == 'start':
                from_sketch_button = True
                thefile = open('./Multiscale/templates/Saved Time.txt', 'w')
                thefile.close()
            ...
            if type(res) == type([]):
                ...
                thefile = open(
                    './Multiscale/templates/Saved Configuration_'+dt_string+'.dcfg', 'w')
                thefile.write(str(res).replace("'", '"'))
                response = make_response(jsonify(res), 200)
                return response
            # print('____IM DOING SOMENTHING___')
            # imagePath = './temp/0v4.jpg'
            # markerPath = './marker'
            # img = cv2.imread(imagePath)
            # gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
